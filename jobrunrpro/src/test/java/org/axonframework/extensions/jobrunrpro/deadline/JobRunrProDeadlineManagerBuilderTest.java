...
class JobRunrProDeadlineManagerBuilderTest {
    ...
    private org.axonframework.deadline.jobrunr.JobRunrProDeadlineManager.Builder builder;
    ...
    void whenAllPropertiesAreSetCreatesManager() {
        org.axonframework.deadline.jobrunr.JobRunrProDeadlineManager manager =
    ...
    void cancelImplemented() {
        org.axonframework.deadline.jobrunr.JobRunrProDeadlineManager manager = builder.scopeAwareProvider(
                                                                                              scopeAwareProvider)
                                                                                      .transactionManager(
                                                                                              transactionManager)
                                                                                      .jobScheduler(jobScheduler)
    ...
    void cancelAllWithinScopeImplemented() {
        JobRunrProDeadlineManager manager = builder.scopeAwareProvider(scopeAwareProvider)
                                                   .transactionManager(transactionManager)
                                                   .jobScheduler(jobScheduler)
                                                   .storageProvider(storageProvider)
                                                   .serializer(TestSerializer.JACKSON.getSerializer())
